...
WORK="$(mktemp -d /tmp/dropshelf-background.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT
xcrun swiftc -O -target arm64-apple-macosx13.0 -sdk "$SDK_PATH" \
  -module-cache-path /tmp/dropshelf-swift-modules -o "$WORK/regression" \
