...
WORK="$(mktemp -d /tmp/dropshelf-operations.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT
xcrun swiftc -target arm64-apple-macosx13.0 -sdk "$SDK_PATH" -module-cache-path /tmp/dropshelf-swift-modules -o "$WORK/operations" \
  "$ROOT"/Sources/Models/*.swift "$ROOT"/Sources/Services/*.swift \
  "$ROOT"/Sources/UI/*.swift "$ROOT"/Sources/AppDelegate.swift \
  "$ROOT"/Tests/Operations/main.swift \
  -framework Cocoa -framework SwiftUI -framework Quartz -framework QuickLookThumbnailing \
  -framework UniformTypeIdentifiers -framework ImageIO -framework ServiceManagement \
  -framework PDFKit -framework Vision -framework CoreML
"$WORK/operations"
